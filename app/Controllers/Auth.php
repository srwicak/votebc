...
class Auth extends BaseController
{
    ...
    {
        try {
            $tempToken = $this->request->getHeaderLine('Authorization');
            if (!$tempToken || !strpos($tempToken, 'Bearer ') === 0) {
                return $this->sendError('Token tidak valid', 401);
            }
            
            $tempToken = substr($tempToken, 7);
            $jwt = new \App\Libraries\JWT();
            $decoded = $jwt->decode($tempToken);
            
            if (!$decoded || !isset($decoded['user_id']) || !isset($decoded['requires_otp']) || !$decoded['requires_otp']) {
                return $this->sendError('Token tidak valid', 401);
            }
            ...
            if (!$otp) {
                return $this->sendError('OTP harus diisi', 400);
            }
            
            $userModel = new UserModel();
            if (!$userModel->verifyOTP($userId, $otp)) {
                return $this->sendError('OTP tidak valid atau sudah kedaluwarsa', 401);
            }
            
            // OTP valid, buat token autentikasi
            $user = $userModel->find($userId);
            unset($user['password']);
    ...
    public function enableTwoFactor()
    {
        try {
            ...
            $userModel = new UserModel();
            $userModel->enableTwoFactor($user['id']);
